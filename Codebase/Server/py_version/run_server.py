...
def filter_sort_data(documents, last_timestamp, device_name):
    data = []
    ...
    last_read_timestamps[device_name] = largest_timestamp
...
if __name__ == '__main__':
    ...
    db_pointer.collection('chair_data').document('SonicWaves-C-001').collection('detections').on_snapshot(chair_snapshot)
    db_pointer.collection('chair_data').document('SonicWaves-C-002').collection('detections').on_snapshot(chair_snapshot)
    db_pointer.collection('chair_data').document('SonicWaves-C-003').collection('detections').on_snapshot(chair_snapshot)
